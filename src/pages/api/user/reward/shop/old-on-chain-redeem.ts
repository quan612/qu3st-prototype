...
const getSignerWallet = (provider, chain, network) => {

  if(chain === Chain.Ethereum){
    if(network === Network.EthereumMainnet){
      new ethers.Wallet(`0x${process.env.OPERATION_WALLET_PRIVATE_KEY}`, provider)
    }
  ...
  if(chain === Chain.Avalance){
    if(network === Network.AvalanceMainnet){
      new ethers.Wallet(`0x${process.env.OPERATION_WALLET_PRIVATE_KEY}`, provider)
    }

    if(network === Network.AvalanceFuji){
      new ethers.Wallet(`0x${process.env.OPERATION_WALLET_PRIVATE_KEY}`, provider)
    }
  }
}
...
const getProvider = (chain, network) => {
